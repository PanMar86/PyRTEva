...
def display_evaluation_report(data_container, status_bar, evaluation_panel):
    ...
    dose_constraints_filepath = str(Path(__file__).resolve().parents[1]/"assets"/"dose_constraints"/"treatment_site"
                               /f"{data_container["TreatmentSite"]}"/f"{data_container["FractionationScheme"]}"/"dose_constraints.pkl")

    with open(dose_constraints_filepath, mode="rb") as f:
